...
export default class Web3ApiHelper  {
  ...
  constructor(mongoInterface, poolConfig ){
   ...
   update(){
    ...
    cron.schedule('*/20 * * * * *', async function() {
      ...
      Web3ApiHelper.fetchAPIData(this.web3,this.poolConfig,this.mongoInterface)
  ...
  static async fetchAPIData(web3,poolConfig,mongoInterface){
    ...
    try{  
      ...
      let gasPrice = await Web3ApiHelper.getGasPriceDataFromBSC(web3);
  ...
  static async getGasPriceDataFromBSC(web3)
  {
    return new Promise((resolve, reject) => {
      web3.eth.getGasPrice().then(data=>{
        let priceEstimates = parseInt(data);
        if(priceEstimates){
          resolve( priceEstimates / 1e9 );
        }else{
          reject(false)
        }
      });
