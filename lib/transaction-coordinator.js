...
//types are 'batched_payment','solution'
   static async addTransactionToQueue(txType, txData,  mongoInterface, poolConfig)
  {
    ...
    var packetData = {
     ...
     // fixed: Multiple challenge_digests correspond to one challenge_number. Lead to gas waste and affect the efficiency of other transactions
     var challengeNumberTx = await TransactionHelper.findOneTransactionWithQuery({"txType":txType,"txData.challenge_number":txData.challenge_number,"block": blockNum},mongoInterface) 
     console.log("query transctions exists status for block : " + blockNum + ",result : ",challengeNumberTx);
     if(!challengeNumberTx){
